...
HourInput:
	...
	call ReadDec ; reads hour into EAX
	mov ch, al ; stores hour into ch
	cmp ch, 0
	jb InvalidInput ; if hour < 0, jump to InvalidInput
	cmp ch, 23
	ja InvalidInput ; if hour > 23, jump to InvalidInput

MinuteInput:
	mov edx, OFFSET enterMinute
	call WriteString ; promps user to enter a minute
	call ReadDec ; reads minute into EAX
	mov cl, al ; stores minute into cl
	cmp cl, 0
	jb InvalidInput ; if minute < 0, jump to InvalidInput
	cmp cl, 59
	ja InvalidInput ; if minute > 59, jump to InvalidInput
...
Calculate:
	...
	cmp ah, 0
	jne OverflowError ; if AH has bits (value is not an 8bit value), then jump to OverflowError
	...
	; bl now contias total minutes for end pair
	; cl now contias total minutes for start pair
	cmp bl, cl
	jb MismatchError ; if endTime < startTime, jump to MismatchError
